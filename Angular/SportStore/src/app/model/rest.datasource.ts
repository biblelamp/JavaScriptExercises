...
export class RestDataSource {
  ...
  saveProduct(product: Product): Observable<Product | Product[]> {
    return this.sendRequest(RequestMethod.Post, "products",
      product, true);
  }

  updateProduct(product): Observable<Product | Product[]> {
    return this.sendRequest(RequestMethod.Put,
      `products/${product.id}`, product, true);
  }

  deleteProduct(id: number): Observable<Product | Product[]> {
