	...
	if ($f == 'file') {
		...
		if ($pth['file'][$file] != '') {
			...
			if ($action == 'download') {
							...
							if (is_array($v1)) {
								foreach($v1 as $k2 => $v2) {
									if (!is_array($v2)) {
										...
										if (get_magic_quotes_gpc())$GLOBALS[$a][$k1][$k2] = stripslashes($GLOBALS[$a][$k1][$k2]);
				...
				chkfile($file, true);
				$title = ucfirst($tx['action']['edit']).' '.$tx['filetype'][$file];
				$o .= '<h1>'.$title.'</h1><form action="'.$sn.'" method="post">';
				if ($form == 'array') {
